...
#include "LiteMath.h"
#include <random>
#include <C:\OpenGL\glflow\glfw-3.3.4.bin.WIN32\include\GLFW\glfw3.h>
#pragma comment(lib, "C:/OpenGL/glflow/glfw-3.3.4.bin.WIN32/lib-vc2019/glfw3_mt.lib")
...
int main()
{
   ...
   // Конфигурация шейдеров
    ...
    GLuint vaoCone;
    GLsizei coneIndices = CreateCone(vaoCone, 30, float3(0.0f, 0.0f, 0.0f), 0.4f, 0.1f);/*
    GLuint vaoCone;
    GLsizei coneIndices = CreateCone(vaoCone, 30, float3(0.0f, 2.25f, -9.55f), 0.4f, 0.1f);*/

    GLuint vaoCylinder;
    GLsizei cylinderIndices = CreateCylinder(vaoCylinder, 30, float3(0.0f, 0.0f, 0.0f), 0.5f, 0.3f);
    /*GLuint vaoCylinder;
    GLsizei cylinderIndices = CreateCylinder(vaoCylinder, 30, float3(0.0f, 2.6f, -10.0f), 0.5f, 0.3f);*/

    GLuint vaoPlane;
    GLsizei planeIndices = CreatePlane(vaoPlane, float3(0.0f, 0.0f, 0.0f), 10.0f);/*
    GLuint vaoPlane;
    GLsizei planeIndices = CreatePlane(vaoPlane, float3(0.0f, -0.8f, -10.0f), 10.0f);*/

    GLuint vaoParallelepiped;
    GLsizei parallelepipedIndices = CreateParallelepiped(vaoParallelepiped, float3(0, 0, 0), 1.0f, 1.0f, 10.0f);/*
    ...
    {
        btCollisionShape* groundShape = new btBoxShape(btVector3(btScalar(5.), btScalar(0.01), btScalar(5.)));

        collisionShapes.push_back(groundShape);

        btTransform groundTransform;
        groundTransform.setIdentity();
        groundTransform.setOrigin(btVector3(0, -0.8, -10));
...

    {
        ...
        //btCollisionShape* colShape = new btBoxShape(btVector3(1,1,1));
        btCollisionShape* colShape = new btSphereShape(btScalar(0.8));
        collisionShapes.push_back(colShape);
        ...
        startTransform.setOrigin(btVector3(0, 6.89, -10));*/
...

    {
        ...
        startTransform.setOrigin(btVector3(0, 20.f, -10));/*
        startTransform.setOrigin(btVector3(0, 8.19, -10));*/
...

    {
        ...
        startTransform.setOrigin(btVector3(0.01, 0.5, -10));/*
        startTransform.setOrigin(btVector3(0, 8.19, -10));*/
    ...
    {
        btCollisionShape* ground1Shape = new btBoxShape(btVector3(btScalar(1.), btScalar(1.), btScalar(10.)));

        collisionShapes.push_back(ground1Shape);

        btTransform groundTransform;
        groundTransform.setIdentity();
        groundTransform.setOrigin(btVector3(-4.5f, -0.3f, -10.0f));
        ...
        btVector3 localInertia(0, 0, 0);
        if (isDynamic)
            ground1Shape->calculateLocalInertia(mass, localInertia);

        //using motionstate is optional, it provides interpolation capabilities, and only synchronizes 'active' objects
        btDefaultMotionState* myMotionState = new btDefaultMotionState(groundTransform);
        btRigidBody::btRigidBodyConstructionInfo rbInfo(mass, myMotionState, ground1Shape, localInertia);
        btRigidBody* body = new btRigidBody(rbInfo);
    ...
    {
        btCollisionShape* ground2Shape = new btBoxShape(btVector3(btScalar(1.), btScalar(1.), btScalar(10.)));

        collisionShapes.push_back(ground2Shape);
        ...
        btVector3 localInertia(0, 0, 0);
        if (isDynamic)
            ground2Shape->calculateLocalInertia(mass, localInertia);

        //using motionstate is optional, it provides interpolation capabilities, and only synchronizes 'active' objects
        btDefaultMotionState* myMotionState = new btDefaultMotionState(groundTransform);
        btRigidBody::btRigidBodyConstructionInfo rbInfo(mass, myMotionState, ground2Shape, localInertia);
        btRigidBody* body = new btRigidBody(rbInfo);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int j = dynamicsWorld->getNumCollisionObjects() - 1; j >= 0; j--)
        {
            ...
            positions.push_back(vec3(float(trans.getOrigin().getX()), float(trans.getOrigin().getY()), float(trans.getOrigin().getZ())));
        ...
        glBindVertexArray(vaoSphere);
        {
            ...
            model1 = translate(model1, vec3(positions[5].x, positions[5].y-0.2, positions[5].z));/*
            ...
            model2 = translate(model2, vec3(positions[4].x, positions[4].y - 0.2 - 0.5, positions[4].z));/*
            ...
            model3 = translate(model3, vec3(positions[3].x, positions[3].y - 0.2 - 0.5 - 0.35, positions[3].z));/*
            ...
            model4 = translate(model4, vec3(positions[2].x, positions[2].y, positions[2].z));
            model4 = scale(model4, vec3(0.5f, 0.5f, 0.5f));
        ...
        {
            mat4 model4 = mat4(1.0f);
            model4 = translate(model4, vec3(positions[3].x-0.15, positions[3].y - 0.2 - 0.5 - 0.35 + 0.1, positions[3].z+0.44));/*
            ...
            model5 = translate(model5, vec3(positions[3].x + 0.15, positions[3].y - 0.2 - 0.5 - 0.35 + 0.1, positions[3].z + 0.44));/*
        ...
        glBindVertexArray(vaoCone); 
        {
            ...
            model = translate(model, vec3(positions[3].x, positions[3].y - 0.2 - 0.5 - 0.35, positions[3].z + 0.45));
            shader.setMat4("model", model);
        ...
        {
            glm::mat4 model = mat4(1.0f);
            model = translate(model, vec3(positions[3].x, positions[3].y - 0.2 - 0.5 - 0.35 + 0.35, positions[3].z));
            shader.setMat4("model", model);
        ...
        {
            glm::mat4 model = mat4(1.0f);
            model = translate(model, positions[6]);
            shader.setMat4("model", model);
